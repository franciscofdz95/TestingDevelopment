...
@Component({
  ...
  styleUrl: './printed.component.css',
  imports: [
    CommonModule,
    FormsModule,
    MatFormFieldModule,
...
export class PrintedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.executeService.execute$
      ...
      .subscribe(event => {
        ...
        if (event.mainTab === 'Bills' && event.subTab === 'Printed') {
  ...
  executecall(params: Paramlist): void {
    console.log('Execute call in Bills - Printed');
  ...
  exportData() {
    console.log('Exporting data from Printed');
  }
}
