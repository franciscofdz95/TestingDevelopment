...
export class ArchivedComponent implements OnInit, OnDestroy {
    ...
    this.executeService.execute$
      ...
      .subscribe(event => {
       // console.log('tabs: ', event);
        if (event.mainTab === 'Bills' && event.subTab === 'Archived') {
  ...
  executecall(params: Paramlist): void {
    console.log('Execute call in Bills - Archived');
  ...
  columnDefs: ColDef[] = [
    ...
    { headerName: 'Assigned Fol', field: 'assignedfol', width: 110 },
    ...
    { headerName: 'Batch Id', field: 'batchid', width: 110 },
  ...
  exportData() {
    console.log('Exporting data from Archived');
  }
}
