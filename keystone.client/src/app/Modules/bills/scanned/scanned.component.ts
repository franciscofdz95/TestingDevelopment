...
@Component({
  ...
  styleUrl: './scanned.component.css',
  imports: [
    CommonModule,
    FormsModule,
    MatFormFieldModule,
...
export class ScannedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.executeService.execute$
      ...
      .subscribe(event => {
        ...
        if (event.mainTab === 'Bills' && event.subTab === 'Scanned') {
  ...
  executecall(params: Paramlist): void {
    console.log('Execute call in Location Bills - Scanned');
  ...
  columnDefs: ColDef[] = [
    ...
    { headerName: 'Remit ID', field: 'remitid', width: 110 },
    ...
    { headerName: 'Scan Destination', field: 'scandest', width: 110 },
    ...
    { headerName: 'Paid', field: 'paid', width: 110 }
  ];

  exportData() {
    console.log('Exporting data from Scanned');
  }
}
