...
export class VerifiedComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.executeService.execute$
      ...
      .subscribe(event => {
        ...
        if (event.mainTab === 'Bills' && event.subTab === 'Verified') {
  ...
  executecall(params: Paramlist): void {
    console.log('Execute call in Bills - Verified');
  ...
  columnDefs: ColDef[] = [
    { headerName: '', field: 'noname', hide: false },
  ...
  exportData() {
    console.log('Exporting data from Verified');
  }
}
